...
class ModelClipartClipart extends Model {
	...
	public function getSource($id_clipart)
	{
		...
		if($query->row) {
			$source[] = array(
						'link' => HTTP_IMAGE.'data/cliparts/'.$query->row['vector_file'], 
						'description' => $query->row['vector_file']
					);
			if($query->row['vector_file_2']) {
				$source[] = array(
							'link' => HTTP_IMAGE.'data/cliparts/'.$query->row['vector_file_2'], 
							'description' => $query->row['vector_file_2']
						);
			}
			$source[] = array(
						'link' => HTTP_IMAGE.'data/cliparts/'.$query->row['swf_file'], 

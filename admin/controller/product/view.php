...
class ControllerProductView extends Controller {
	...
	public function form($data = array()) {
		...
		if (!empty($shade)) { 
			...
			$this->data['shade_url'] = HTTPS_IMAGE . 'data/products/' . $shade;
		} else {
			...
			$this->data['shade_url'] = HTTPS_IMAGE . 'data/products/default_shade.png';
		}
		...
		/*if (!empty($fills)) { 
			foreach($fills as $key=>$fill) {
				$this->data['fills'][] = array(
					'view_fill_index' => $key, 
					'fill_file' => $fill, 
					'image' =>  HTTPS_IMAGE . 'data/products/'.$fill,
			...
			$this->data['fills'][] = array(
				...
				'image' =>  HTTPS_IMAGE . 'data/products/default_fill.png',
  	...
  	public function fill($data = array()) {
		...
		if (isset($data['fill_file'])) { 
			$this->data['image'] = HTTPS_IMAGE . 'data/products/'.$data['fill_file'];
		} else {
			$this->data['image'] = HTTPS_IMAGE . 'data/products/default_fill.png';
		}
	...
	public function upload_shade() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && !isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				{
					$json['filename'] = $file;
					$json['image'] = HTTPS_IMAGE . 'data/products/' . $file;

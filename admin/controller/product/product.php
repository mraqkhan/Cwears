  	...
  	private function getList() {	
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('product/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
			...
			if(!is_null($filter_printable) && $filter_printable=="0") {
				if(isset($printable_ids[$result['product_id']])) {	
					continue;
				}
			}
			...
			$this->data['products'][] = array(
				...
				'printable'  => (isset($printable_ids[$result['product_id']]) ? $this->language->get('text_enabled') : $this->language->get('text_disabled')),
